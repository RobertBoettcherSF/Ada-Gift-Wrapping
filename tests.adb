--  Standalone test suite for Gift_Wrapping (main program).
...
with Ada.Text_IO;
with Gift_Wrapping; use Gift_Wrapping;
...
procedure Tests is
   ...
   function Raised_Invalid_Jarvis (Pts : Point_Set) return Boolean is
   begin
      declare
         H : constant Point_Array := Jarvis_March (Pts);
      begin
         pragma Unreferenced (H);
         return False;
      end;
...
begin
   Ada.Text_IO.Put_Line ("Gift_Wrapping tests");
   ...
   Check (Raised_Invalid_Jarvis (Empty_Set), "Jarvis empty raises");
   Check (Raised_Invalid_Hull (Too_Many), "Hull oversized raises");
   Check (Raised_Invalid_Andrew (Too_Many), "Andrew oversized raises");
   Check (Raised_Invalid_Jarvis (Too_Many), "Jarvis oversized raises");
   ...
   begin
      ...
      declare
         T : constant Point_Array := Jarvis_March (One);
      begin
         Check (Same_Vertex_Set (H1 (1 .. N1), T), "Jarvis alias matches single");
      end;
   end;
   ...
   declare
      ...
      J : constant Point_Array := Jarvis_March (Tri);
   begin
      ...
      Check (Same_Vertex_Set (H, J), "triangle matches Jarvis alias");
   ...
   ------------------------------------------------------------------
   Section ("14. Start is leftmost-then-lowest on hull");
   ------------------------------------------------------------------
   declare
      ...
      H : constant Point_Array := Convex_Hull (Pts);
   begin
      --  Leftmost x is x=0 → (0,2).
      Check (H'Length >= 3, "leftmost case hull ≥ 3");
      Check (Near_Point (H (H'First), P (0.0, 2.0)),
             "hull starts at leftmost (0,2)");
      Check (Is_CCW (H), "leftmost case CCW");
      Check (Is_Strictly_Convex_CCW (H), "leftmost case strictly convex");
   end;

   ------------------------------------------------------------------
   Section ("15. Jarvis_March alias identity");
   ------------------------------------------------------------------
   declare
      Pts : constant Point_Set :=
   ...
   begin
      Check (Same_Vertex_Set (H, J), "Jarvis_March ≡ Convex_Hull set");
      Check (H'Length = J'Length, "Jarvis_March same length");
      Check (Is_CCW (J), "Jarvis_March result CCW");
   end;
